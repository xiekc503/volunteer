...
{
   m_stmt = NULL;
   memset(strSql_buf, 0 , sizeof(strSql_buf));
}
...
CSqlStatement::CSqlStatement(LPCTSTR statement, sqlite3* db)
{
   ...
   CCodingConv::GB2312_2_UTF8(strSql_buf, sizeof(strSql_buf), statement);
   int rc = _sqlite3_prepare(db, strSql_buf, -1, &m_stmt, 0 );
...
CSqlStatement::Bind(int pos_zero_indexed, LPCTSTR value)
{
   ...
   CCodingConv::GB2312_2_UTF8(value_buf, sizeof(value_buf), value);

   int rc = _sqlite3_bind_text(m_stmt, pos_zero_indexed+1,
                             value_buf, strlen(value_buf), SQLITE_TRANSIENT);
...
int
CSqlStatement::getInt(int pos_zero_indexed)
{
   return _sqlite3_column_int(m_stmt, pos_zero_indexed);
}

LPCTSTR
CSqlStatement::getString(int pos_zero_indexed)
{
   m_szText.Format(_T("%s"), _sqlite3_column_text(m_stmt, pos_zero_indexed));
   CCodingConv::UTF8_2_GB2312(value_buf, sizeof(value_buf), m_szText);
...
{
   bool fResult = TRUE;
   CCodingConv::GB2312_2_UTF8(m_szName, 1024, db_file, 0);
   int rc = _sqlite3_open(m_szName, &m_db);
...
void CDbSQLite::ReleaseStatement(CSqlStatement* stmt)
{
	if( stmt )
	{
		delete stmt;
